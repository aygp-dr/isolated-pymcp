...
def benchmark_prime_algorithms(n: int) -> None:
    ...
    if n <= 100000:  # Only run for smaller values
        ...
        print(f"primes_up_to({n}): Found {len(result)} primes (Time: {end - start:.6f}s)")
    
    start = time.time()
    result = sieve_of_eratosthenes(n)
    end = time.time()
    print(f"sieve_of_eratosthenes({n}): Found {len(result)} primes (Time: {end - start:.6f}s)")
    
    start = time.time()
    result = segmented_sieve(n)
    end = time.time()
    print(f"segmented_sieve({n}): Found {len(result)} primes (Time: {end - start:.6f}s)")
